...
class Modal extends HTMLElement {
  modalData: ModalData = {
    ...
    [ModalType.UPGRADE]: {
      ...
      contentText: `<li class="${listItemStyle}">
      <div>${tick}</div><div>Ad-free</div>
    </li>
    <li class="${listItemStyle}">
      <div>${tick}</div><div>Commercial Use</div>
    </li>
    <li class="${listItemStyle}">
      <div>${tick}</div><div>Unlimited workspaces</div>
    </li>
    <li class="${listItemStyle}">
      <div>${tick}</div><div>Priority support</div>
    </li>`,
      buttonIcon: "",
      buttonText: "Upgrade to Pro",
  ...
  connectedCallback() {
    ...
    this.innerHTML = `<div class="${wrapperStyle}"><div class="${style}">${header}${description}${content}${buttonContainer}${
      customFooter ?? ""
    }</div></div>`
...
const tick = `<svg width="1rem" height="1rem" viewBox="0 0 36 33" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const githubSVG = `<?xml version="1.0" encoding="UTF-8" standalone="no"?>
  <svg class="inline-block mr-6" width="3rem" height="3rem" viewBox="0 0 256 250" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" preserveAspectRatio="xMidYMid">
      <g>
...
const listItemStyle =
  "grid grid-flow-col items-center grid-cols-feature gap-x-5"
